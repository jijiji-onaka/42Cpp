...
/*   Updated: 2021/07/10 06:16:06 by tjinichi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	Bureaucrat::signForm(Form &form)
{
	...
	{
		std::cout << RED""BOLD << this->name_ << " cannot sign " << form.getName() \
			<< " because ";
		std::cerr << e.what() << '\n';
		return (false);
	}
...
bool	Bureaucrat::executeForm(Form const &form)
{
	...
	{
		form.execute(*this);
		std::cout << GREEN_3""BOLD << this->name_ << " executes " << form.getName() \
			<< RESET << std::endl;
	}
	catch(const std::exception& e)
	{
		std::cout << RED""BOLD << this->name_ << " cannot execute " << form.getName() \
			<< " because ";
		std::cerr << e.what() << '\n';
		return (false);
	}
